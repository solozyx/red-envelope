...
func (domain *goodsDomain) preCreateItem(dto services.RedEnvelopeReceiveDTO) {
	...
	//	s = "碰运气"
	//}
	//domain.itemDomain.Desc = fmt.Sprintf("%s的%s红包", goods.Username.String, s)
}
...
func (domain *goodsDomain) transfer(ctx context.Context, dto services.RedEnvelopeReceiveDTO) (status services.TransferredStatus, err error) {
	...
	if target.AccountNo == "" {
		a := accounts.NewAccountDomain().GetAccountByUserIdAndType(target.UserId, services.EnvelopeAccountType)
	...
	//	TradeBody:   target,
	//	TradeTarget: body,
	//	Amount:      domain.itemDomain.Amount,
	//	ChangeType:  services.EnvelopeIncoming,
	//	ChangeFlag:  services.FlagTransferIn,
	//	Desc:        "抢红包成功收入",
	//}
